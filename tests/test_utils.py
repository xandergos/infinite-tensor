...
def assert_tensor_properties(
    ...
    if expected_dtype is not None:
        assert tensor.dtype == expected_dtype, (
            f"Dtype mismatch: got {tensor.dtype}, expected {expected_dtype}"
        )

    if expected_min is not None:
        actual_min = tensor.min().item()
        assert abs(actual_min - expected_min) <= tolerance, (
            f"Min value mismatch: got {actual_min}, expected {expected_min}"
        )

    if expected_max is not None:
        actual_max = tensor.max().item()
        assert abs(actual_max - expected_max) <= tolerance, (
            f"Max value mismatch: got {actual_max}, expected {expected_max}"
        )

    if expected_mean is not None:
        actual_mean = tensor.mean().item()
        assert abs(actual_mean - expected_mean) <= tolerance, (
            f"Mean value mismatch: got {actual_mean}, expected {expected_mean}"
        )


def create_dependency_chain(
    base_tensor: InfiniteTensor, transform_func: Callable, window: TensorWindow, chain_length: int
) -> InfiniteTensor:
...
def generate_test_data(
    shape: tuple[int, ...], data_type: str = "random", seed: Optional[int] = None
) -> torch.Tensor:
...
def benchmark_tensor_operation(
    operation: Callable[[], torch.Tensor], num_iterations: int = 10, warmup_iterations: int = 3
) -> dict:
