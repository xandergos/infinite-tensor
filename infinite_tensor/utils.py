...
def standardize_indices(
    tensor_shape: tuple[int | None, ...], indices: tuple[int | slice, ...]
    ...
    if Ellipsis in indices:
        ...
        expanded_indices = (
            indices[:ellipsis_idx] + (slice(None),) * n_missing + indices[ellipsis_idx + 1 :]
        )
    else:
    ...
    if len(expanded_indices) < len(tensor_shape):
        expanded_indices = expanded_indices + (slice(None),) * (
            len(tensor_shape) - len(expanded_indices)
        )
        ...
        if shape_dim is not None:
            ...
            if isinstance(idx, int):
                ...
                if idx < 0 or idx >= shape_dim:
                    raise IndexError(
                        f"Index {idx} is out of bounds for dimension with size {shape_dim}"
                    )
                ...
                start = (
                    ...
                    else (shape_dim + idx.start if idx.start < 0 else idx.start)
                )
                stop = (
                    idx.stop
                    if idx.stop is None
                    else (shape_dim + idx.stop if idx.stop < 0 else idx.stop)
                )
    ...
    normalized_slices = [
        normalize_slice(idx, shape_dim) for idx, shape_dim in zip(processed_indices, tensor_shape)
    ]
